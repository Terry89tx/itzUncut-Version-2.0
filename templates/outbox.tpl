...
<tr>
        ...
        <TD>
                ...
                <tr bgcolor="{cycle values="#eeeeee,#f5f5f5"}">
                        ...
                        <td><a href="{$baseurl}/msg.php?id={$pm_id[i]}">{$subject[i]}</a></td>
                        <td>{$receiver[i]}</td>
                        <td>{$date[i]|date_format:"%A, %B %e, %Y"}
                        <td width=10><a href="{$baseurl}/outbox.php?id={$pm_id[i]}&action=del" onclick="javascript:return confirm('Are you sure you want to delete this message?');">Delete</a></td>
                </td>
                </tr>
                {/section}
                {if $pm_id eq ""}
