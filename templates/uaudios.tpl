...
<TD style="PADDING-RIGHT: 15px">
        ...
        <TD width=550>
                ...
                audios {$start_num}-{$end_num} of {$total} </DIV>
                {insert name=id_to_name assign=uname un=$smarty.request.UID}
                <SPAN>{if $smarty.request.type eq "private"}Private audios{else}Public audios{/if} // {$uname}</SPAN>
                </DIV></DIV>
                ...
                <A href="{$baseurl}/view_audio.php?viewkey={$answers[i].vkey}">{$answers[i].title}</A>
                </DIV>
        ...
        <A href="{$baseurl}/invite_friends.php">Share your audios with friends!</A>
        </DIV>
        </TD>
        <TD></TD>
        </TR>
