...
<TBODY>
        ...
        <TR vAlign=top>
				  ...
				  <img src="images/nv_2_l.jpg" width="5" height="24" alt=""></td>
					  ...
					  {if $smarty.request.category eq ""}{assign var="catgy" value=mr}{else}{assign var="catgy" value=$smarty.request.category}{/if}
					   {insert name=tag_to_name assign=tagname tag=$catgy}
					   {$tagname}
					  </td>
                                          <td align=center width="300" >
					  {if $smarty.request.viewtype eq "detailed"}
					        <A href="{$baseurl}/audio.php?page={$page}&viewtype=basic"><span class="white_regular">Basic View</span></A>
				        {else}		
				        <STRONG>
						<span class="white_regular">Basic View</span>
					</STRONG>
				        {/if} | {if $smarty.request.viewtype eq "" or $smarty.request.viewtype eq "basic"}
					        <A href="{$baseurl}/audio.php?page={$page}&viewtype=detailed"><span class="white_regular">Detailed View</span></A>
				        {else}
					        <STRONG><span class="white_regular">Detailed View</span></STRONG>
					{/if}</td>
					<td  class="white_regular" align=right> 
						audios {$start_num}-{$end_num} of {$total}
					</td>
                                          </tr>
                                      </table>
                                  </div></th>
        ...
        <A href="{$baseurl}/view_audio.php?viewkey={$answers[i].vkey}&page={$page}&viewtype={$smarty.request.viewtype}&category={$catgy}">
      <IMG  height=70 src="{$tmburl}/audio.png" border=0v>
        ...
        <DIV class=moduleFeaturedDetails>
		 ...
		 Added: 
			{insert name=time_range assign=rtime field=addtime IDFR=VID id=$answers[i].VID tbl=audio}{$rtime}
		 <BR>
			by {insert name=id_to_name assign=uname un=$answers[i].UID}
	  ...
	  {insert name=show_rate assign=rate rte=$answers[i].rate}
        ...
        <A href="{$baseurl}/view_audio.php?viewkey={$answers[i].vkey}&page={$page}&viewtype={$smarty.request.viewtype}&category={$catgy}">
        <IMG  height=70 src="{$tmburl}/audio.png"></A>
        </TD>
        ...
        <DIV class=moduleFeaturedDetails>
        Added: 
	{insert name=time_range assign=rtime field=addtime IDFR=VID id=$answers[i].VID tbl=audio}{$rtime} by
