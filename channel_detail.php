...
if($rs->recordcount()>0)
{
        ...
        $users = $rs->getrows();
        $sql="SELECT count(VID) as total, UID from video WHERE channel like '%|$_REQUEST[chid]|%'  group by UID order by total desc limit 5";
        $rsx = $conn->Execute($sql);
        $tmpary = $rsx->getrows();
        STemplate::assign('actuser',$tmpary);

        $sql="SELECT * from video WHERE channel like '%|$_REQUEST[chid]|%' order by addtime desc limit 5";
        $rsx = $conn->Execute($sql);
        $tmpary = $rsx->getrows();
...
$sql="SELECT * from video WHERE channel like '%|$_REQUEST[chid]|%' order by viewnumber desc limit 5";
$rsx = $conn->Execute($sql);
$tmpary = $rsx->getrows();
